...
- `release/1.x-legacy` 维护线第二十八个补丁版本
...
- `release/1.x-legacy` 维护线第二十四个补丁版本

### Bugs Fixes

- **现在可以和官方版 Clash Verge Rev 同时打开了**:之前两个软件用了同一个内部检测端口,先开哪个,后开的就会被当成"已经在跑的实例"自动退出,所以你只能开一个。本版换了端口跟官方错开,两个一起开互不影响

- **关闭服务模式时,TUN 开关现在会跟着自动关掉**:之前关服务模式只关了服务,TUN 那个开关在配置里还是开着,下次启动可能出现"配置说要 TUN、但服务没了"的别扭状态。现在两个一起关,干净
