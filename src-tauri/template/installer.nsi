...
!macro CheckAllVergeProcesses
  ; Check if the legacy Clash Verge service is running
  !if "${INSTALLMODE}" == "currentUser"
    nsis_tauri_utils::FindProcessCurrentUser "${VERGE_SERVICE_PROCESS}"
  !else
    nsis_tauri_utils::FindProcess "${VERGE_SERVICE_PROCESS}"
  !endif
  Pop $R0
  ${If} $R0 = 0
    DetailPrint "Kill ${VERGE_SERVICE_PROCESS}..."
    !if "${INSTALLMODE}" == "currentUser"
      nsis_tauri_utils::KillProcessCurrentUser "${VERGE_SERVICE_PROCESS}"
    !else
      nsis_tauri_utils::KillProcess "${VERGE_SERVICE_PROCESS}"
    !endif
  ${EndIf}
